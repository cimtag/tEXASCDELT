...
public class ExecSCD1a2JUnit extends AbstractTestBase {
	...
	public final void test1() {
		...
		g.addSourceColumn("scd1a", "VARCHAR", 30, 0, false, false, true, false, false, false, null);
		...
		try {
			...
			assertEquals("b", rs.getString("SCD1A"));
			assertEquals("c", rs.getString("SCD2"));
			rs.next();
			assertEquals("aa", rs.getString("SCD0"));
			assertEquals("bb", rs.getString("SCD1A"));
	...
	static void createTable(String sourceSchema, String sourceTable) {
		String sql = "create table " + sourceSchema + "." + sourceTable
				+ " (the_key DECIMAL(30,0) NOT NULL, val_start DATE, scd0 VARCHAR(30), scd1a VARCHAR(30), scd2 VARCHAR(30))";

		execute(sql);
		sql = "insert into " + sourceSchema + "." + sourceTable + " values "
				+ "(42,'2017-01-02','a','x','c'),(42,'2017-01-03','x','b','c'),"
				+ "(42,'2017-01-04','aa','y','cc'),(42,'2017-01-05','aa','bb','cc')";
